...
def run_experiment(dataset_name, n_instances=10, n_folds=10, timeout_sec=60):
    ...
    for fold, rf_model in enumerate(rf_models):
        ...
        for instance_idx, instance_dict in enumerate(instances):
            ...
            explainer_RF = Explainer.initialize(
                rf_model, 
                instance, 
            ...
            th = explainer_RF.get_theory()
            v_list = explainer_RF.binary_representation
            target_prediction = explainer_RF.target_prediction
            forme_cnf = explainer_RF._random_forest.to_CNF(
        explainer_RF._instance, 
        explainer_RF._binary_representation,
        target_prediction, 
        tree_encoding=Encoding.MUS
            ...
            res_dict.update({
                ...
                "len_feat_ext": len(explainer_RF.to_features(cpi_xp_to_sat_format(res_ext))) if status_ext == "Success" else None
            })
            
            # Method 2: PyXAI CPI-XP
            ordre_features = []
            func_cpi = lambda: explainer_RF.cpi_xp(
                n=1, 
            ...
            res_dict.update({
                ...
                "len_feat_cpi": len(explainer_RF.to_features(res_cpi)) if status_cpi == "Success" else None
            })
            
            # Method 3: PyXAI M-CPI-XP
            func_mcpi = lambda: explainer_RF.m_cpi_xp(
                n=1, 
                strategy="priority_order", 
                random_seed=None, 
                ordre_features=ordre_features
            ...
            res_dict.update({
                ...
                "len_feat_mcpi": len(explainer_RF.to_features(res_mcpi)) if status_mcpi == "Success" else None
            })
            
            # Method 4: PyXAI Sufficient Reason (Baseline)
            func_sr = lambda: explainer_RF.sufficient_reason()
            ...
            res_dict.update({
                ...
                "len_feat_sr": len(explainer_RF.to_features(res_sr)) if status_sr == "Success" else None
            })
