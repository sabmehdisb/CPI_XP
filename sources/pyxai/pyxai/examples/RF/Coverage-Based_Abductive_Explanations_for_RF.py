...
import time
import os
import statistics  # <--- Added for mean/std calculation
import pandas as pd
...
def display_summary(report):
    """
    Prints a formatted summary table to the console, including standard deviations.
    """
    print("\n" + "="*100)
...
def run_experiment(dataset_name, n_instances=10, n_folds=10, timeout_sec=60):
    ...
    details = rf_learner.get_details()
    ...
    for fold, rf_model in enumerate(rf_models):
        ...
        for instance_idx, instance_dict in enumerate(instances):
            ...
            func_sr = lambda: explainer.sufficient_reason()
    ...
    # Return 3 values: results, dataset stats, and model performance
...
def main():
    ...
    # Run experiment (passing timeout_sec=1 as in your previous snippet, adjust if needed)
